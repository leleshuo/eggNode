...
class PassController extends Controller {
  ...
  async doLogin() { //登录时查询手机号，如果手机号不存在则注册，引入手机号的概念
    const username = this.ctx.request.body.username;
    const password = this.ctx.request.body.password;
    console.log(this.ctx.request.body);
    let msg = '成功',code = 0,type='success';
    const userInfo = new this.ctx.model.User({
      username,
      password
    });
  ...
  async treeData(){
    ...
    let msg = [{"categoryName": "车","categoryNum": "3"},{"categoryName": "车1","categoryNum": "1"}];
    this.ctx.body = {
      success: false,
      code: '0',
      type:'type',
